...
async function emitirCertificado(params){
  try {
    ...
    if (paciente_rut){
      ...
      if (pa && pa[0]) {
        // Nombre completo sin duplicar apellidos (la carga masiva dejó el nombre
        // completo en 'nombre', y algunos registros también tienen apellido/apellido2).
        var _base = (pa[0].nombre || '').trim();
        var _aps = [pa[0].apellido, pa[0].apellido2].filter(Boolean).join(' ').trim();
        nombre = !_aps ? _base : (_base.toLowerCase().endsWith(_aps.toLowerCase()) ? _base : (_base + ' ' + _aps).trim());
      }
    }
    ...
    // Instrumentado: capturamos status + cuerpo crudo para ver el motivo real si falla.
    let rawText = '';
    let out = null;
    try { rawText = await ins.text(); out = JSON.parse(rawText); } catch (e) { /* out queda null */ }
    if (out && out[0]) return { ok:true, codigo: out[0].codigo, id: out[0].id };
    return { ok:false, motivo:'insert-fallo', status: ins.status, respuesta: String(rawText || '').slice(0, 400) };
