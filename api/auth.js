...
// Reconstruye el RUT al formato EXACTO que usa Medilink: "8900518-3"
// (cuerpo + guión + dígito verificador, sin puntos, sin cero a la izquierda).
// Confirmado en vivo: Medilink solo hace match con este formato; sin guión da 0.
function rutCanonico(rut){
  const limpio = (rut||'').replace(/[^0-9kK]/g, '').toUpperCase(); // "89005183"
  if(limpio.length < 2) return null;
  const cuerpo = String(parseInt(limpio.slice(0, -1), 10)); // quita ceros a la izquierda
  const dv     = limpio.slice(-1);
  if(!cuerpo || cuerpo === 'NaN') return null;
  return cuerpo + '-' + dv;          // "8900518-3"
}

async function medilinkBuscarRut(rut){
  const canon = rutCanonico(rut);
  const limpio = (rut||'').replace(/\./g,'').replace(/\s/g,'').trim();
  const candidatos = [...new Set([canon, limpio].filter(Boolean))]; // canónico primero
