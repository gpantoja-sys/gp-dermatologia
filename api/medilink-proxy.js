...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (tipo === 'rut') {
      // Medilink solo hace match con el RUT canónico "8900518-3"
      // (con guión, sin puntos, sin cero a la izquierda) y filtra por la
      // columna 'rut'. Reconstruimos ese formato desde lo que llegue.
      const limpio = valor.replace(/[^0-9kK]/g, '').toUpperCase();
      const cuerpo = String(parseInt(limpio.slice(0, -1), 10));
      const dv = limpio.slice(-1);
      const canon = (limpio.length >= 2 && cuerpo !== 'NaN') ? cuerpo + '-' + dv : valor;
      const candidatos = [...new Set([canon, valor.replace(/\./g, '').trim()])];
