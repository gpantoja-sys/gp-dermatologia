...
// ─────────────────────────────────────────────────────────────────────────────
// Crea UNA transacción Webpay Plus (un leg) y la registra en transbank_transacciones.
// Para el split de dos empresas, el cliente arma los legs y envía el primero +
// 'pendientes' (los legs que faltan). El retorno encadena el siguiente.
// Lleva además el ENGANCHE: accion_id / presupuesto_id / sesion_id, para que el
// retorno pueda cerrar la cola de Clarita al quedar el grupo autorizado.
//   · SANDBOX (por defecto): credenciales PÚBLICAS de integración.
//   · PRODUCCIÓN: TBK_ENV=production + TBK_SKINTOUCH_* / TBK_LASERTOUCH_* en Vercel.
// ─────────────────────────────────────────────────────────────────────────────
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const grupo = body.grupo || ('G-' + Date.now());
    const pendientes = Array.isArray(body.pendientes) ? body.pendientes : [];
    // Enganche con la cola (todos opcionales)
    const accion_id      = body.accion_id || null;
    const presupuesto_id = body.presupuesto_id || null;
    const sesion_id      = body.sesion_id || null;
    if (!monto || monto < 1) { res.status(400).json({ error: 'monto inválido' }); return; }

    const buyOrder  = 'GP-' + Date.now() + '-' + Math.floor(Math.random() * 1000);
    ...
    // 2) Registrar el leg (estado 'iniciada') con grupo + pendientes + enganche
    ...
    await sbFetch('transbank_transacciones', {
      ...
      body: JSON.stringify({
        ...
        estado: 'iniciada',
        payload: {
          empresa, concepto, token: tbk.token, session_id: sessionId, paciente_rut,
          grupo, pendientes, accion_id, presupuesto_id, sesion_id
        }
      })
    });

    res.status(200).json({ token: tbk.token, url: tbk.url, buy_order: buyOrder, grupo: grupo });
  } catch (e) {
