...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const grupo = body.grupo || ('G-' + Date.now());                          // une los legs de una atención
    const pendientes = Array.isArray(body.pendientes) ? body.pendientes : [];  // legs que faltan tras este
    ...
    // 2) Registrar el leg (estado 'iniciada'), guardando grupo + pendientes para el encadenado
    ...
    await sbFetch('transbank_transacciones', {
      ...
      body: JSON.stringify({
        ...
        empresa_id: emp_id,
        estado: 'iniciada',
        payload: { empresa, concepto, token: tbk.token, session_id: sessionId, paciente_rut, grupo, pendientes }
      })
    });
